...
def get_delivery_note_weights(sales_invoice):
    ...
    for dn_item in dn_items:
        ...
        if key in grouped_items:
            ...
            grouped_items[key]["total_weight"] += dn_item["custom_weight"]
        else:
            grouped_items[key] = {
                "item_code": dn_item["item_code"],
                "qty": dn_item["qty"],
                "total_weight": dn_item["custom_weight"]
            }
    ...
    for key, grouped_item in grouped_items.items():
        ...
        new_item.custom_weight_in_kg = grouped_item["total_weight"]
        new_item.qty = grouped_item["qty"]
        new_item.rate = 0  # ✅ Keep rate 0 since user will enter custom_rate_per_kg manually
